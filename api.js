...
function SearchMatches(text,heroes){
    ...
    return heroes.filter(Myhero=>{
        const rexex = new RegExp(text,"gi");
        return Myhero.name.match(rexex);
    });
}
...
search.addEventListener("input",()=>{
   ...
   const html = matches.map(Myhero=>{
       return`<li class="matches" val="${Myhero.id}"> ${Myhero.name}</li>`;
...
search_btn.addEventListener("click",()=>{
    ...
    window.open(`Myhero.html?id=${id}`,"_blank");
})
